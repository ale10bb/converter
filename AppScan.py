...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(prog='AppScan', description='读取AppScan的XML文件，转换成测评能手标准格式，转换结果分别保存至每个原始文件的同目录下。')
    ...
    for item in target:
        try:
            ...
            logger.debug('fix-recommendation-group: {}'.format(list(fixRecommendationGroup.keys())))
            # 漏洞种类
            threatClassGroup = {}
            ...
            logger.debug('security-risk-group: {}'.format(list(securityRiskGroup.keys())))
            # 漏洞涉及url
            urlGroup = {}
            ...
            for issueTypeNode in srcXMLRoot.findall('./issue-type-group/item[@id]'):
                ...
                try:
                    ...
                    assert key
                    ET.SubElement(vulRoot, 'SOLUTION').text = fixRecommendationGroup[key]
                except:
                    logger.warning('"{}" 未读取到 "./fix-recommendation/ref"'.format(issueName))
                ...
                try:
                    ...
                    logger.debug('【{}】 has {} entities'.format(issueName, len(issues[issueTypeNode.get('id')])))
                except:
